...
class ProviderEnum(StrEnum):
    ...
    STEAM — Steam-аккаунты (maFile, Guard-коды, деавторизация сессий); CHAT —
    шаренные аккаунты с TOTP-кодом и ручной деавторизацией. Новые игры внутри
    провайдера — это данные (подкатегории), новый провайдер — это новый код.
    """
...
@unique
class ChatRentalStatusEnum(StrEnum):
    """Статус аренды Chat (мультитенант: несколько аренд на одном аккаунте).

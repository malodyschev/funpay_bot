...
class NewOrderService:
    ...
    async def handle(self, event: NewOrderEvent) -> None:
        ...
        if provider == ProviderEnum.CHAT:
            await self._handle_chat_order(event, lots[0])
            return
    ...
    async def _handle_chat_order(self, event: NewOrderEvent, lot: Lot) -> None:
        """Заказ Chat: подобрать аккаунт пула (least-loaded) и выдать login+password."""
        chat_svc = ChatRentalService(self.session, self.deps)
        result = await chat_svc.place(
            lot,
            buyer_id=event.buyer_id,
            buyer_username=event.buyer_username,
            chat_id=event.chat_id,
        ...
        await self.deps.funpay.send_message(event.chat_id, text)
        # Заняли слот → если пул заполнился, скрыть лоты категории на FunPay.
        if lot.category_id is not None:
            await chat_svc.sync_pool_visibility(lot.category_id)
        logger.info('chat rental created for order %s on account %s', event.order_id, account.id)
